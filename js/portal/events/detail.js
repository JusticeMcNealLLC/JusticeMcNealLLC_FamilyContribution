...
async function evtOpenDetail(eventId) {
    ...
    window.__evtTeamToolsCtx = {
        ...
        canAccessTeamHub,
        canCreateTeamChat: isCreator || (typeof canManageEvents === 'function' && canManageEvents()),
    };
...
function evtCanCreateTeamChat(event) {
...
function evtTeamChatFriendlyError(err) {
...
function evtTeamChatDisplayName(profile) {
    if (!profile) return 'Member';
    const name = `${profile.first_name || ''} ${profile.last_name || ''}`.trim();
    return name || 'Member';
}

function evtTeamChatFormatTime(iso) {
    if (!iso) return '';
    try {
...
async function evtEnsureTeamChat(event, eventId) {
...
async function evtLoadTeamChatMessages(chatId, eventId) {
...
function evtTeamChatMessagesHtml(state) {
    ...
    return state.messages.map(m => {
        ...
        const name = evtEscapeHtml(evtTeamChatDisplayName(profile));
        const time = evtEscapeHtml(evtTeamChatFormatTime(m.created_at));
...
function evtRefreshTeamChatMessageList() {
    const state = window.__evtTeamChatState;
    const el = document.getElementById('evtTeamChatMessages');
    if (!state || !el) return;
    el.innerHTML = evtTeamChatMessagesHtml(state);
    el.scrollTop = el.scrollHeight;
}

function evtHandleTeamChatRealtime(payload) {
    const state = window.__evtTeamChatState;
...
function evtSubscribeTeamChat(chatId, eventId) {
    const state = window.__evtTeamChatState;
    if (!state) return;
    if (state.channel) {
        try { supabaseClient.removeChannel(state.channel); } catch (_) { /* ignore */ }
    ...
    state.channel = supabaseClient
        ...
        }, (payload) => evtHandleTeamChatRealtime({ eventType: 'INSERT', new: payload.new }))
        .on('postgres_changes', {
            event: 'UPDATE',
            schema: 'public',
            table: 'event_chat_messages',
            filter: `event_id=eq.${eventId}`,
        }, (payload) => evtHandleTeamChatRealtime({ eventType: 'UPDATE', new: payload.new }))
        .subscribe();
}

function evtRenderTeamChatPanel(eventId, opts) {
    ...
    if (loading) {
        ...
        const state = window.__evtTeamChatState;
        body = `
            <div id="evtTeamChatMessages" class="evt-team-chat-messages">${evtTeamChatMessagesHtml(state)}</div>
...
async function evtOpenTeamChat(eventId) {
    ...
    if (!bar) bar = evtEnsureCtaBarShell();
    evtApplyDesktopTeamToolsOverlay(bar);
    ...
    panel.innerHTML = `${closeBtn}${evtRenderTeamChatPanel(eventId, { loading: true })}`;

    const ensure = await evtEnsureTeamChat(event, eventId);
    if (ensure.error) {
        panel.innerHTML = `${closeBtn}${evtRenderTeamChatPanel(eventId, {
            unavailable: evtTeamChatFriendlyError(ensure.error),
        })}`;
        return;
    }
    if (ensure.notStarted || !ensure.chat) {
        panel.innerHTML = `${closeBtn}${evtRenderTeamChatPanel(eventId, { notStarted: true })}`;
        return;
    }

    const chatId = ensure.chat.id;
    const loaded = await evtLoadTeamChatMessages(chatId, eventId);
    if (loaded.error) {
        panel.innerHTML = `${closeBtn}${evtRenderTeamChatPanel(eventId, {
            unavailable: evtTeamChatFriendlyError(loaded.error),
        })}`;
        return;
    }
    ...
    panel.innerHTML = `${closeBtn}${evtRenderTeamChatPanel(eventId, { canCompose: true })}`;
    evtRefreshTeamChatMessageList();
    evtSubscribeTeamChat(chatId, eventId);
    ...
    if (input) {
        input.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' && !e.shiftKey) {
                e.preventDefault();
                evtSendTeamChatMessage(eventId);
            }
        });
    }
}

async function evtSendTeamChatMessage(eventId) {
    const state = window.__evtTeamChatState;
    ...
    if (error) {
        if (statusEl) statusEl.textContent = evtTeamChatFriendlyError(error);
        return;
    }
...
function evtBuildTeamToolsPanelHtml(event, eventId, rsvp, myRaffleEntry, entriesClosed, eventIsFull, opts) {
    ...
    rows.push(evtTeamToolsRow('Team Chat', 'Private team coordination', `evtCloseCtaPanel();evtOpenTeamChat('${eventId}')`, false));
...
function evtApplyDesktopTeamToolsOverlay(bar) {
    ...
    if (!bar.dataset.evtOverlayCloseBound) {
        ...
        bar.addEventListener('click', (e) => {
            if (e.target === bar) evtCloseCtaPanel();
        });
    }
}
...
window.evtOpenTeamChat          = evtOpenTeamChat;
window.evtSendTeamChatMessage   = evtSendTeamChatMessage;
window.evtCleanupTeamChat       = evtCleanupTeamChat;
...
detail.openTeamChat        = evtOpenTeamChat;
