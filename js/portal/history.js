...
async function loadTransactions(userId, filters = {}) {
    ...
    try {
        ...
        invoices.forEach(inv => {
            const isExtraDeposit = inv.payment_type === 'extra_deposit';
            transactions.push({
                source: 'stripe',
        ...
        if (tableBody) {
            tableBody.innerHTML = transactions.map(tx => {
                ...
                const isExtra = tx.isExtraDeposit;
                const isOneTime = isDeposit || isExtra;

                let sourceBadge;
                if (isExtra) {
                    sourceBadge = '<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-violet-100 text-violet-700"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Extra Deposit</span>';
                } else if (isDeposit) {
                ...
                const rowBorder = isOneTime ? 'border-l-[3px] border-l-violet-400' : '';
                ...
                return `<tr class="hover:bg-gray-50 transition ${rowBorder}">
                    ...
                    <td class="px-5 py-4 text-sm font-semibold ${tx.status === 'paid' ? (isOneTime ? 'text-violet-600' : 'text-emerald-600') : 'text-gray-900'}">+${formatCurrency(tx.amount)}</td>
        ...
        if (mobileList) {
            mobileList.innerHTML = transactions.map(tx => {
                ...
                const icon = isOneTime ? depositIcon : monthlyIcon;
                const iconBg = isOneTime ? 'bg-violet-100' : 'bg-emerald-100';
                const label = isExtra ? 'Extra deposit' : isDeposit ? `One-time · ${tx.depositType}` : 'Monthly payment';
                const sublabel = isDeposit ? (tx.notes || formatDate(tx.date)) : formatDate(tx.date);
                const amountColor = tx.status === 'paid' ? (isOneTime ? 'text-violet-600' : 'text-emerald-600') : 'text-gray-600';
                const borderClass = isOneTime ? 'border-l-[3px] border-l-violet-400' : '';
