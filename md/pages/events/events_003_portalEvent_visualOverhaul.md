...
> **Status:** Phase A1+A2+A3+B1+B2+B3+B4+B5+C1+C2+C3+C4 shipped. **Phase D opened** — see [events_004_portalEvent_phaseD.md](events_004_portalEvent_phaseD.md) (D3 + D1 + D2 shipped; **D4 dark mode REMOVED at user request**, SW at `v68`). **Phase E partial:** E1 (gradient header) + E2 (mobile search pill) + E3 (inline category chip rail) + E4 (emoji bucket labels) + E5 (Top Picks rail) + E6 (festival hero + CTA bar) + E7 (hero attendee cluster) + E9 (motion polish) + E10 (in-header notification bell + unread dot) + E11 (per-bucket See all / Show less) + E12 (hero heart favorite → RSVP 'maybe') shipped — **vlift default-ON live**, opt-out via `?vlift=0`. Phase A4 (polish/QA on real device) still pending — non-blocking for Phase C since C1 is mobile-gated behind `ontouchstart` + `innerWidth < 640` + `scrollY === 0`. **Phase E (Premium Visual Lift) scoped** — see §Phase E + Appendix C for the design-parity plan against the California gradient + Tomorrowland festival mockups.
...
E7  ✅ SHIPPED  Hero attendee cluster (up-to-5 avatars + N going pill, glass background, click → detail) — reuses `window.evtAttendees`, no new query — SW `v65 → v66`
E8  ❌ REMOVED  Dark-mode-compatible CSS tokens — D4 dark mode torn out at user request, SW `v67 → v68`
E9  ✅ SHIPPED (partial)  Motion polish (header fade, chip press) — ken-burns + stagger pending
E10 ✅ SHIPPED  In-header notification bell + red unread dot (mirrors global #notifBadge, forwards click to #notifBtn) — SW `v62 → v63`
E11 ✅ SHIPPED  "See all" / "Show less" per-bucket truncation (vlift, threshold 6) — SW `v61 → v62`
E12 ✅ SHIPPED  Hero heart-favorite (top-right glass pill, fill rose-500 when active) — enum lacks 'interested', so mapped to existing `status='maybe'` (already wired as ❤️ Interested in detail.js). Reuses `evtHandleRsvp` toggle. Hero-only scope; cards deferred — SW `v66 → v67`
