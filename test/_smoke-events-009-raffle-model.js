...
const manageDocs = read('js/portal/events/manage/docs.js');
...
const createSheetIdx = chain.indexOf('create/sheet.js');
const manageSheetIdx = chain.indexOf('manage/sheet.js?v=113');
assert(modelIdx >= 0, 'portal chain should load raffle model helper');
assert(modelIdx < listIdx, 'raffle model should load before list/shell.js');
assert(modelIdx < createSheetIdx, 'raffle model should load before create sheet');
assert(modelIdx < manageSheetIdx, 'raffle model should load before manage sheet');
assert(html.includes('events.bundle.js'), 'portal events.html should load events.bundle.js');
assert(!html.includes('classic-chain-loader.js'), 'production should not load classic-chain-loader in HTML');
const publicModelScriptIndex = publicHtml.indexOf('../js/portal/events/core/raffle-model.js');
...
assert(/data-ec-raffle-add-category/.test(raffleBuilder), 'raffle-builder should support adding categories');
assert(/data-ec-raffle-add-item/.test(raffleBuilder), 'raffle-builder should support adding items');
assert(/validateConfig\(config\)/.test(raffleBuilder), 'raffle-builder should validate raffle config');
assert(/raffleModel\(\)\.validateConfig\(rb\.ensureRaffleConfig\(\)\)/.test(createSheet), 'create sheet should validate via raffle builder API');
assert(/Number\(f\.raffle_entry_cost_dollars \|\| 0\) < 0/.test(createSheet), 'create sheet should allow zero-dollar raffle entries');
assert(/raffle_prizes: raffleConfig/.test(createSubmit), 'create submit should store v2 raffle prizes');
assert(/raffle_winner_count: raffleWinnerCount/.test(createSubmit), 'create submit should store raffle winner count');
...
assert(/EventsManage\.open/.test(detailSections), 'detail sections should reference EventsManage.open for host navigation');
assert(!/Draw Raffle Winners/.test(detail), 'detail page should not expose inline draw winners button');
assert(/id="emRaffleDrawBtn"/.test(manageRaffle), 'manage raffle tab should render draw button');
assert(/evtOpenRaffleDraw\?\.\(STATE\.eventId, STATE\.event\)/.test(manageRaffle), 'manage raffle tab should open draw modal with event context');
assert(/Raffle command/.test(manageRaffle), 'manage raffle tab should use command-center copy');
assert(/eligibleEntries\.length/.test(manageRaffle), 'manage raffle tab should count eligible entries');
assert(/guestByToken/.test(manageRaffle), 'manage raffle tab should resolve guest winner names');
assert(/Document handoff/.test(manageDocs), 'manage docs tab should use command-center copy');
assert(/Attendance command/.test(manageRsvps), 'manage RSVPs tab should use command-center copy');
assert(/Money command/.test(manageMoney), 'manage money tab should use command-center copy');
assert(/Danger zone/.test(manageDanger), 'manage danger tab should use command-center copy');
assert(/event_type !== 'competition'/.test(manageComp), 'manage comp tab should skip competition queries for non-competition events');
assert(/events:raffle:drawn/.test(manageRaffle), 'manage raffle should refresh after winner draw');
assert(/refreshRaffle/.test(manageSheet), 'manage sheet should expose raffle refresh after draws');
assert(/function winnerBelongsToCategory\(/.test(manageRaffle), 'manage raffle should count legacy winners by category fallback');
assert(/function winnerChoiceHtml\(/.test(manageRaffle), 'manage raffle should render winner-choice assignment controls');
assert(/function assignWinnerChoice\(/.test(manageRaffle), 'manage raffle should assign pending-choice winners');
assert(/selection_status: 'assigned'/.test(manageRaffle), 'winner-choice assignment should mark winners assigned');
assert(/FOR UPDATE/.test(winnerChoiceMigration), 'winner-choice migration should add an update policy');
assert(/function evtDetailRafflePrizesHtml\(/.test(detailRaffleRender), 'detail raffle-render should render v2 prize categories');
assert(/function evtDetailRaffleWinnersHtml\(/.test(detailRaffleRender), 'detail raffle-render should render compact winners');
...
assert(/\.event-detail-surface \.event-detail-grid\s*{[^}]*display:\s*block/s.test(detailCss), 'shared event mobile grid should become single-column');
assert(/\.event-detail-surface \.event-detail-rail\s*{[^}]*display:\s*none/s.test(detailCss), 'shared event mobile sidebar should not reserve layout width');
