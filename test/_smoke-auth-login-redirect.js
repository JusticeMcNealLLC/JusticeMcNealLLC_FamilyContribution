...
assert(/target\.pathname\.startsWith\('\/auth\/'\)/.test(loginJs), 'login should block redirect loops back into auth pages');
...
assert(/\/auth\/login\.html\?redirect=/.test(publicEventJs), 'public event page should link sign-in with redirect');
